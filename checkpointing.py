%%writefile train_with_checkpointing.py
import os
import math
import pickle
import time
...
class CheckpointDecoderLayer(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, layer, key_padding_mask):
        ctx.layer = layer
        ctx.key_padding_mask = key_padding_mask
        with torch.no_grad():
            output, _ = layer(x, key_padding_mask=key_padding_mask, output_attentions=False)
...
class DecoderOnlyTransformer(nn.Module):
    def __init__(self, vocab_size, d_model, num_layers, num_heads, max_len, 
                 dropout=0.1, embedding_weights=None, freeze_embeddings=False, 
    ...
    def forward(self, input_ids, attention_mask=None,output_attentions=False):
        ...
        if self.use_checkpointing:
            ...
            for layer in self.layers:
                x = CheckpointDecoderLayer.apply(x, layer, key_padding_mask)
        else:
            for layer in self.layers:
                if output_attentions:
...
def main():
    parser = argparse.ArgumentParser(description="DDP Training with Manual Checkpointing")
    parser.add_argument('--checkpoint', action='store_true', 
                        help="Enable manual gradient checkpointing")
    ...
    if rank == 0:
        print(f"--- RUNNING WITH CHECKPOINTING: {args.checkpoint} ---")
    ...
    if rank == 0: print(f"Loading embedding matrix...")
    with open(EMBEDDING_MATRIX_SAVE_PATH, "rb") as f:
        embedding_matrix = pickle.load(f)

    if rank == 0: print("Loading tokenized dataset...")
    ...
    if rank == 0: print("Building Sampler & Loader")
    train_sampler = DistributedSampler(train_dataset, shuffle=True)
    train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE,
                              sampler=train_sampler, collate_fn=collate_fn,
                              num_workers=4, pin_memory=True)

    if rank == 0: print("Starting Model")
    ...
    model = DDP(
        ...
        find_unused_parameters=args.checkpoint
    )
    ...
    for step, (input_ids, target_ids, _) in pbar:
        ...
        with torch.amp.autocast(device_type='cuda', dtype=torch.float16):
            logits,_ = model(input_ids, attention_mask=attention_mask, output_attentions=False) 
    ...
    if rank == 0:
        ...
        print("\n" + "="*50)
        print(f"--- FINAL REPORT (Checkpointing: {args.checkpoint}) ---")
        print(f"Epoch {epoch} Train Loss: {train_loss:.4f}")
        print(f"Epoch {epoch} Runtime: {epoch_duration:.2f}s")
        print(f"Epoch {epoch} Peak VRAM: {peak_mem:.2f} GB")
        print("="*50)
